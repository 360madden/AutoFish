...
Plan-backed `one-cast` and `bounded-session` manifests expose a normalized `reviewGates` object so summaries can consistently report `planFresh`, `targetCurrent`, `fishabilityCandidate`, and `oneCast` gate status. They also expose `sessionPlanProvenance` so summaries show the session-plan path, review scope token, fishability-fan source candidate, and any attached operational-facing evidence. Older top-level gate fields remain for compatibility.
